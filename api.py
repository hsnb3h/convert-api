...
Html = PyDocX.to_html('mydoc.docx')
f = open('from-docx.html', 'w', encoding='utf-8')
f.write(Html)
f.close()
File = open('from-docx.html', 'a')

with fileinput.FileInput('from-docx.html', inplace=True) as file:
    for line in file:
        print(line.replace('<html>', '<html lang="ar" dir="rtl">'))

f = open('from-docx.html', 'r')
codebase = f.read()
f.close()
questions = []
text = codebase.split('______________________________________________________________________')
text.pop(0)
encoded_list = []
